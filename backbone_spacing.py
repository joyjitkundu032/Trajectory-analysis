...
for CA1 in carbons:
	...
	n=0
	for CA2 in carbons:
		if carbon_segments[m] == carbon_segments[n]:
			pass
		elif math.fabs(correct_distance(CA1[2],CA2[2],2)) < 10:
			pass
		else:
			distance = calculate_distance(CA1[0],CA2[0],CA1[1],CA2[1],CA1[2],CA2[2])
			if distance < distance_min:
				distance_min = distance
			else:
				pass
		n+=1
	if distance_min < 12.0:
		print(m,carbon_segments[m])
	m+=1
	distances.append(distance_min)

data = open(str(myfile.name) + '_backbone_distance.txt', 'w')
i=0
